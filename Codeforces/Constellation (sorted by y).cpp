...
int main(){
	...
	rep(i, n){
		cin >> pos[i].fi.se >> pos[i].fi.fi; //try to sort by y instead
		pos[i].se = i;
	}
	sort(all(pos));
	rep(i, n - 2){
		pi one = {pos[i].fi.se, pos[i].fi.fi};
		pi two = {pos[i + 1].fi.se, pos[i + 1].fi.fi};
		pi three = {pos[i + 2].fi.se, pos[i + 2].fi.fi};
