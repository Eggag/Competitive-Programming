...
struct seg{
	...
	seg(int s){
		...
		for(int i = 0; i < (1 << 18); i++){
			dat[i] = (node){(ll)1e18, 1};
		}
	}
	node comb(node a, node b){
		if(a.mn < b.mn) return a;
		else if(b.mn < a.mn) return b;
		else return (node){a.mn, a.num + b.num};
	}
	void update(int i, int s, int e, int x, ll v){
		...
		dat[i] = comb(dat[i << 1], dat[i << 1 | 1]);
	}
	void update(int x, ll v){
		update(1, 0, sz, x, v);
	}
	node query(int i, int s, int e, int x, int y){
		if(e < x || y < s) return (node){(ll)1e18, 1};
		if(x <= s && e <= y) return dat[i];
		int m = (s + e) / 2;
		return comb(query(i << 1, s, m, x, y), query(i << 1 | 1, m + 1, e, x, y));
	}
