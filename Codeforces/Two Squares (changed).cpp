...
int main(){
	...
	rep(i, 4){
		...
		if(a[i].fi >= b[0].fi && a[i].fi <= b[3].fi &&
			...
			if(a[i].se <= (b[0].se + (a[i].fi - b[0].fi))) cur++;
			if(a[i].se >= (b[1].se + (b[1].fi - a[i].fi))) cur++;
			if(a[i].se <= (b[3].se + (b[3].fi - a[i].fi))) cur++;
			if(a[i].se >= (b[1].se + (a[i].fi - b[1].fi))) cur++;
		}
		if(cur == 4){
			cout << "YES" << endl;
			return 0;
	...
	if(b[0].fi < a[0].fi && b[3].fi > a[2].fi &&
		...
		if((a[0].fi > b[1].fi) || (a[3].fi < b[1].fi)) cout << "NO" << endl;
		else cout << "YES" << endl;;
		return 0;
	}
	cout << "NO" << endl;
